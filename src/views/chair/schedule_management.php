...
<div class="min-h-screen bg-gray-100 font-sans">
                                ...
                                foreach ($times as $time) {
                                    ...
                                    foreach (['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'] as $day) {
                                        ...
                                        if ($schedule) {
                                            ...
                                            echo "<strong class='text-gray-900'>" . htmlspecialchars($schedule['course_code'] ?? '') . " - " . htmlspecialchars($schedule['course_name'] ?? '') . "</strong><br>";
                                            echo "<span class='text-gray-600 text-xs'>" . htmlspecialchars($schedule['faculty_name'] ?? 'N/A') . "</span><br>";
                                            echo "<span class='text-gray-600 text-xs'>" . (isset($schedule['room_name']) ? htmlspecialchars($schedule['room_name']) : 'Online') . "</span><br>";
                                            echo "<span class='text-gray-600 text-xs'>" . htmlspecialchars($schedule['section_name'] ?? '') . " (" . htmlspecialchars($schedule['schedule_type'] ?? '') . ")</span>";
                                            echo "<div class='mt-2'>";
 ...
 <form method="POST" action="/chair/schedule_management" class="grid grid-cols-1 gap-6" id="generate-form">
    ...
    <?php
    $filterSections = array_unique(array_column($schedules, 'section_name'));
            ...
            <div class="px-2 py-2 border-r border-gray-200 last:border-r-0 min-h-[<?php echo $rowSpan * 80; ?>px] relative" data-day="<?php echo $day; ?>" data-time="<?php echo $time[0]; ?>" data-end-time="<?php echo $time[1]; ?>">
                ...
                foreach ($schedulesForSlot as $schedule) {
                    ...
                    if ($scheduleStart === $time[0]) {
                        $colors = ['bg-blue-100 border-blue-300 text-blue-800', 'bg-green-100 border-green-300 text-green-800', 'bg-purple-100 border-purple-300 text-purple-800', 'bg-orange-100 border-orange-300 text-orange-800', 'bg-pink-100 border-pink-300 text-pink-800'];
                        $colorClass = $colors[array_rand($colors)];
                ?>
                        <div class="<?php echo $colorClass; ?> p-2 rounded-lg border-l-4 mb-1" data-year-level="<?php echo htmlspecialchars($schedule['year_level'] ?? 'N/A'); ?>" data-section-name="<?php echo htmlspecialchars($schedule['section_name'] ?? 'N/A'); ?>">
                            <div class="font-semibold text-xs truncate mb-1"><?php echo htmlspecialchars($schedule['course_code'] ?? ''); ?></div>
                            <div class="text-xs opacity-90 truncate mb-1"><?php echo htmlspecialchars($schedule['section_name'] ?? ''); ?></div>
                            <div class="text-xs opacity-75 truncate"><?php echo htmlspecialchars($schedule['faculty_name'] ?? 'N/A'); ?></div>
                            <div class="text-xs opacity-75 truncate"><?php echo htmlspecialchars($schedule['room_name'] ?? 'Online'); ?></div>
                            <div class="text-xs font-medium mt-1"><?php echo date('g:i A', strtotime($schedule['start_time'])) . ' - ' . date('g:i A', strtotime($schedule['end_time'])); ?></div>
                        </div>
                <?php
                    }
                }
...
<script>
    ...
    const sectionsData = Array.isArray(rawSectionsData) ? rawSectionsData : Object.values(rawSectionsData).map((name, index) => {
        let yearLevel = 'Unknown';
        const parts = name.split('-');
        if (parts.length > 1) {
            yearLevel = parts[1].trim(); // Take the part after '-', e.g., '1A' from 'BSIT-1A'
        }
        return {
            section_id: index + 1,
            section_name: name,
            year_level: yearLevel,
            curriculum_id: document.getElementById('generate_curriculum_id').value || null
        };
    });
    ...
    function updateYearLevels() {
        ...
        if (curriculumId && Array.isArray(sectionsData)) {
            const yearLevels = sectionsData.map(s => s.year_level);
            console.log('Extracted Year Levels:', yearLevels); // Debug individual year levels
            const uniqueYears = [...new Set(sectionsData.map(s => s.year_level).filter(y => y && y !== 'Unknown'))];
    ...
    function updateSections() {
        ...
        if (curriculumId && Array.isArray(sectionsData)) {
            let matchingSections = sectionsData.filter(s => s.curriculum_id == curriculumId);
    ...
    const times = [
        ['07:30', '08:30'], ['08:30', '10:00'], ['10:00', '11:00'], ['11:00', '12:30'],
        ['12:30', '13:30'], ['13:00', '14:30'], ['14:30', '15:30'], ['15:30', '17:00'], ['17:00', '18:00']
    ];
