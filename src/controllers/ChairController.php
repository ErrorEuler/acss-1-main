...
class ChairController
{
    ...
    {
        header('Content-Type: text/html; charset=UTF-8');// this is Mask Dont delete this please
    ...
    {
        $url = "http://localhost:8000/api/{$endpoint}.php"; // Use port 8000
        error_log("Calling API: $method $url with data " . json_encode($data));
        $ch = curl_init($url);
        ...
        curl_close($ch);
        error_log("API response: code $httpCode, data " . ($response ?: 'null'));
    ...
    public function manageSchedule() {
        ...
        $departmentId = $this->getChairDepartment($chairId);
        error_log("manageSchedule: departmentId fetched: $departmentId");
        $currentSemester = $this->getCurrentSemester();
        error_log("manageSchedule: currentSemester fetched: " . print_r($currentSemester, true));
        $_SESSION['current_semester'] = $currentSemester;
        $activeTab = $_GET['tab'] ?? 'generate';
        $error = $success = $unassignedWarning = null;
        $scheduleModel = new ScheduleModel($this->db);
        $schedules = $scheduleModel->getSchedulesByFaculty($chairId, $currentSemester['semester_id']) ?: [];

        if ($departmentId) {
            error_log("manageSchedule: Entering if block for departmentId: $departmentId");
            unset($_SESSION['schedule_cache'][$departmentId]);
            if (!isset($_SESSION['schedule_cache'][$departmentId])) {
                error_log("manageSchedule: Cache not set, about to call loadCommonData");
                try {
                    $_SESSION['schedule_cache'][$departmentId] = $this->loadCommonData($departmentId, $currentSemester);
                    error_log("manageSchedule: Cache initialized for dept $departmentId: " . print_r($_SESSION['schedule_cache'][$departmentId], true));
                } catch (Exception $e) {
                    error_log("manageSchedule: Exception in loadCommonData: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
                }
            }
            ...
            $sections = $cachedData['sections'] ?? $this->getSections($departmentId, $currentSemester['semester_name'], $currentSemester['academic_year']);
            error_log("Final sections data: " . print_r($sections, true));

            if ($_SERVER['REQUEST_METHOD'] === 'POST' && $activeTab === 'generate') {
                error_log("POST received in manageSchedule: " . print_r($_POST, true));
                $selectedCurriculumId = $_POST['curriculum_id'] ?? null;
                $selectedYears = $_POST['year_levels'] ?? [];
                $selectedSections = $_POST['sections'] ?? [];
                if ($selectedCurriculumId) {
                    $result = $this->generateSchedules($selectedCurriculumId, $selectedYears, $selectedSections, $departmentId, $currentSemester, $classrooms, $faculty, $scheduleModel);
                    if ($result['success']) {
                        $success = $result['message'];
                        $schedules = array_merge($schedules, $result['schedules']);
                        header("Location: /chair/schedule_management?tab=manual");
                        exit;
                    } else {
                        $error = $result['message'];
                        if (isset($result['unassigned'])) $unassignedWarning = true;
                    }
                } else {
    ...
    {
        error_log("loadCommonData called with departmentId: $departmentId, semester: " . ($currentSemester['semester_name'] ?? 'null') . ", academic_year: " . ($currentSemester['academic_year'] ?? 'null')); // Debug entry
        return [
    ...
    private function getSections($departmentId, $semester, $academicYear)
    {
        try {
            ...
            error_log("getSections query params - department_id: $departmentId, semester: $semester, academic_year: $academicYear");
            $stmt->execute([':department_id' => $departmentId, ':semester' => $semester, ':academic_year' => $academicYear]);
            $sections = $stmt->fetchAll(PDO::FETCH_ASSOC);
            error_log("Retrieved sections count: " . count($sections) . ", data structure: " . json_encode(array_keys($sections[0] ?? []))); // Check keys
            error_log("Retrieved sections data: " . print_r($sections, true));
            return $sections;
        } catch (PDOException $e) {
            error_log("PDOException in getSections: " . $e->getMessage());
            return [];
        }
    }
    ...
    {
        $stmt = $this->db->prepare("
        SELECT course_id, course_code, course_name, units, COALESCE(subject_type, 'General Education') as subject_type 
        FROM courses 
    ...
    private function generateSchedules($curriculumId, $yearLevels, $sectionIds, $departmentId, $currentSemester, $classrooms, $faculty, $scheduleModel) {
        $this->db->beginTransaction();
        $schedules = [];
        try {
            $semesterId = $currentSemester['semester_id'];
            if (!$semesterId) throw new Exception("Invalid semester ID");
    
            $stmt = $this->db->prepare("DELETE FROM schedules WHERE semester_id = :semester_id AND status = 'Pending' AND department_id = :department_id AND section_id IN (" . implode(',', array_map('intval', $sectionIds)) . ")");
            $stmt->execute([':semester_id' => $semesterId, ':department_id' => $departmentId]);
            error_log("Cleared Pending schedules for semester $semesterId, dept $departmentId, sections " . implode(',', $sectionIds));
    
            $courses = $this->getCurriculumCourses($curriculumId);
            error_log("Courses for curriculum $curriculumId: " . print_r($courses, true));
    
            $sections = array_filter(
                $_SESSION['schedule_cache'][$departmentId]['sections'] ?? [],
                fn($s) => in_array($s['section_id'], $sectionIds) && in_array($s['year_level'], $yearLevels) && $s['curriculum_id'] == $curriculumId
            );
            error_log("Matching sections after filter: " . print_r($sections, true));
    
            if (empty($sections) || empty($courses)) {
                $this->db->rollBack();
                return ['success' => false, 'message' => 'No sections or courses available.', 'schedules' => []];
            }
    
            $availableFacultyIds = array_column($faculty, 'faculty_id');
            $availableRoomIds = array_column($classrooms, 'room_id');
            $days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
            $timeSlots = [
                ['07:30:00', '08:30:00'], ['08:30:00', '10:00:00'], ['10:00:00', '11:00:00'],
                ['11:00:00', '12:30:00'], ['12:30:00', '13:30:00'], ['13:00:00', '14:30:00'],
                ['14:30:00', '15:30:00'], ['15:30:00', '17:00:00'], ['17:00:00', '18:00:00']
            ];
            $unassignedCourses = [];
    
            foreach ($courses as $course) {
                if (!in_array(strtolower($course['curriculum_year']), array_map('strtolower', $yearLevels))) {
                    $unassignedCourses[] = $course['course_code'];
                    continue;
                }
    
                $assigned = false;
                foreach ($sectionIds as $sectionId) {
                    if (!isset($sections[$sectionId]) || $sections[$sectionId]['year_level'] !== $course['curriculum_year']) continue;
    
                    shuffle($availableFacultyIds);
                    shuffle($availableRoomIds);
                    shuffle($days);
                    shuffle($timeSlots);
    
                    foreach ($availableFacultyIds as $facultyId) {
                            ...
                            foreach ($days as $day) {
                                foreach ($timeSlots as $slot) {
                                    $scheduleData = [
                                        ...
                                        'schedule_type' => 'F2F',
                                        'day_of_week' => $day,
                                        'start_time' => substr($slot[0], 0, 5),
                                        'end_time' => substr($slot[1], 0, 5),
                                    ...
                                    error_log("Attempting createSchedule with data: " . print_r($scheduleData, true));
                                    $schedulingService = new SchedulingService($this->db);
                                    $result = $schedulingService->createSchedule($scheduleData, $departmentId);
                                    error_log("createSchedule result for course {$course['course_code']}: " . print_r($result, true));
                                    if ($result['code'] === 200) {
                                        $schedules[] = $result['data']['schedule'];
                                        $assigned = true;
                                        break 4;
                                    } elseif ($result['code'] === 409) {
                                        error_log("Conflict for course {$course['course_code']}: " . $result['data']['error']);
                                        continue;
                                    }
                                }
                            }
                    ...
                    if (!$assigned) $unassignedCourses[] = $course['course_code'];
                }
            }
    
            $this->db->commit();
            error_log("Generated schedules count: " . count($schedules));
            return ['success' => true, 'message' => 'Schedules generated successfully!', 'schedules' => $schedules] + 
                   (!empty($unassignedCourses) ? ['unassigned' => $unassignedCourses] : []);
        } catch (Exception $e) {
            $this->db->rollBack();
            error_log("Generate Schedules error: " . $e->getMessage());
            return ['success' => false, 'message' => 'Generation failed: ' . $e->getMessage(), 'schedules' => []];
        }
    }
    ...
    private function hasFacultyConflict($facultyId, $day, $startTime, $endTime)
    {
        $stmt = $this->db->prepare("
            SELECT COUNT(*) FROM schedules 
            WHERE faculty_id = :faculty_id AND day_of_week = :day AND semester_id = :semester_id AND (
    ...
    private function hasConflict($schedules, $day, $startTime, $endTime)
    {
        foreach ($schedules as $schedule) {
            if (
                $schedule['day_of_week'] === $day &&
                !($endTime <= $schedule['start_time'] || $startTime >= $schedule['end_time'])
            ) {
    ...
    {
        $sql = "INSERT INTO schedules (course_id, section_id, room_id, semester_id, faculty_id, schedule_type, day_of_week, start_time, end_time, status, is_public, department_id) VALUES (:course_id, :section_id, :room_id, :semester_id, :faculty_id, :schedule_type, :day_of_week, :start_time, :end_time, :status, :is_public, :department_id)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
    ...
    {
        $stmt = $this->db->prepare("SELECT c.course_id, c.course_code, c.course_name, cc.year_level AS curriculum_year, cc.semester AS curriculum_semester FROM curriculum_courses cc JOIN courses c ON cc.course_id = c.course_id WHERE cc.curriculum_id = :curriculum_id"); // Removed semester filter for testing
        $stmt->execute([':curriculum_id' => $curriculumId]);
        $courses = $stmt->fetchAll(PDO::FETCH_ASSOC);
        error_log("Raw courses from DB for curriculum $curriculumId: " . print_r($courses, true));
        return $courses;
    }
    ...
    private function getRandomTimeSlot($usedSlots, $isMajor = false)
    {
        $startTimes = ['07:30', '08:30', '10:00', '11:00', '12:30', '13:00', '14:30', '15:30'];
        $duration = $isMajor ? 90 : 60; // 90 minutes for major, 60 for others
        do {
            $start = $startTimes[array_rand($startTimes)];
            $end = date('H:i', strtotime($start . ' +' . $duration . ' minutes'));
            $slot = "$start-$end";
        } while (in_array($slot, $usedSlots));
        $usedSlots[] = $slot;
        return ['start_time' => $start . ':00', 'end_time' => $end . ':00'];
    }

    private function hasRoomConflict($schedules, $day, $startTime, $endTime, $roomId)
    {
        foreach ($schedules as $schedule) {
            if ($schedule['day_of_week'] === $day && $schedule['room_id'] === $roomId) {
                ...
                if (!($newEnd <= $scheduleStart || $newStart >= $scheduleEnd)) {
                    return true; // Conflict detected
                }
            }
        }
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                try {
                    ...
                    $data = [
                        ...
                        'department_id' => $departmentId,
                        'subject_type' => $_POST['subject_type'] ?? 'General Education',
