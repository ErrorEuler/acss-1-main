...
{
    try {
        error_log("createSchedule called with data: " . print_r($data, true));
        ...
        // Fetch year level from sections table
        $sectionStmt = $this->db->prepare("SELECT year_level FROM sections WHERE section_id = ?");
        $sectionStmt->execute([$data['section_id']]);
        $yearLevel = $sectionStmt->fetchColumn();
        if (!$yearLevel) {
            return [
                'code' => 400,
                'data' => ['error' => "Invalid section selected."]
            ];
        }

        $curriculumStmt = $this->db->prepare("SELECT curriculum_id, curriculum_name FROM curricula WHERE curriculum_id = ? AND department_id = ?");
        $params = [$data['curriculum_id'], $departmentId];
        error_log("Executing curriculumStmt with params: " . print_r($params, true));
        ...
        $courseStmt = $this->db->prepare("
            ...
            WHERE c.course_id = ? 
            AND cc.curriculum_id = ? 
            AND c.department_id = ?
        ");
        $params = [$data['course_id'], $data['curriculum_id'], $departmentId];
        error_log("Executing courseStmt with params: " . print_r($params, true));
        ...
        $facultyStmt = $this->db->prepare("
            ...
            AND u.department_id = ?
        ");
        $params = [$data['faculty_id'], $departmentId];
        error_log("Executing facultyStmt with params: " . print_r($params, true));
        ...
        $sectionStmt = $this->db->prepare("
            ...
            AND s.curriculum_id = ?
        ");
        $params = [$data['section_id'], $departmentId, $data['curriculum_id']];
        error_log("Executing sectionStmt with params: " . print_r($params, true));
        ...
        if ($data['schedule_type'] !== 'Asynchronous') {
            $roomStmt = $this->db->prepare("
                ...
                FROM classrooms 
                WHERE room_id = ? 
                AND (department_id = ? OR (shared = 1 AND availability = 'available'))
            ");
            $params = [$data['room_id'], $departmentId];
            error_log("Executing roomStmt with params: " . print_r($params, true));
        ...
        try {
            ...
            error_log("ConflictStmt query: " . $conflictStmt->queryString);
            error_log("ConflictStmt params: " . print_r($params, true));
            $conflictStmt->execute($params);
            if ($conflictStmt->fetchColumn() > 0) {
                throw new Exception("Schedule conflict detected for faculty and section.");
            }
            ...
            error_log("InsertStmt params: " . print_r($params, true));
            $insertStmt = $this->db->prepare("
                INSERT INTO schedules (
                    course_id, faculty_id, room_id, section_id, 
                    schedule_type, day_of_week, start_time, end_time, semester_id,
                    department_id, status, approved_by, is_public
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            error_log("Executing query: " . $insertStmt->queryString . " with params: " . print_r($params, true));
            $insertStmt->execute($params);
            error_log("Schedule inserted with ID: " . $this->db->lastInsertId());
            $this->db->commit();
        } catch (PDOException $e) {
            $this->db->rollBack();
            error_log("SchedulingService: Insert failed - " . $e->getMessage());
            return [
                'code' => 500,
            ...
            $this->db->rollBack();
            error_log("SchedulingService: Conflict or validation failed - " . $e->getMessage());
            return [
                'code' => 409,
